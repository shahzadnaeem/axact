...
function ChatControls({ ws, ws_id, ws_username, users }) {
  ...
  return (
    <div class="grid-rows">
      <SendName ws={ws} ws_id={ws_id} ws_username={ws_username} />
      <SentTo users={users} toId={toId} setToId={setToId} />
      <SendMessage
        ws={ws}
        ws_id={ws_id}
...
function ChatMessagesLog({ ws_id, ws_message }) {
  ...
  return (
    <section class="message-log grid-1col">
      ...
      {addLastMessageAnchor && <div class="last-message-anchor"></div>}
    </section>
  );
}
...
export default function Chat({ ws, ws_id, ws_username, ws_message, users }) {
  return (
    <section class="chat grid-2row-a-1fr">
      <ChatControls
        ws={ws}
        ws_id={ws_id}
        ws_username={ws_username}
        users={users}
      />
      <ChatMessagesLog ws_id={ws_id} ws_message={ws_message} />
    </section>
  );
}
