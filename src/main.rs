...
fn print_listening_message(addr: SocketAddr, port: u16) {
    ...
    if sys_name != "Windows" {
        println!("Listening/Old UI http://{addr}... [{sys_name}]");
        println!("Standalone UI    http://0.0.0.0:5173");
    } else {
        // Local connection thing with Windows
        println!("Listening/Old UI http://127.0.0.1:{port}... [{sys_name}]");
        println!("Standalone UI    http://127.0.0.1:5173");
    }
}
